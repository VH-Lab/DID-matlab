function d = finddocs_missing_dependencies(DB, varargin)
    ...
    %
    % Searches the DID database DB and returns all documents that have a
    % dependency ('depends_on') field for which the 'value' field does not
    % correspond to an existing document.
    %
    % The following form:
    %
    % D = FINDDOCS_MISSING_DEPENDENCIES(DB, NAME1, NAME2, ...)
    %
    ...
    d = DB.database_search(ndi_query('depends_on','hasfield','',''));
