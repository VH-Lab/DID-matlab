classdef  postgresdb < database
    ...
    methods, % public
        ...
        function sqlquery = ndiquery_to_sql(did_postgresdb_obj, ndiquery)
            ...
            %
            % q = did_query(‘did_document.id’,‘exact_string’,‘ABCD’,‘’)
            % (This means: find all documents that have a field did_document.id that exactly matches the string ‘ABCD’)
    ...
    methods (Access=protected),
        ...
        function new_db = do_add(did_postgresdb_obj, did_document_obj, add_parameters)
            ...
            sqlwrite(did_postgresdb_obj.db, tablename, did_document_obj)
            new_db = did_postgresdb_obj
