...
verifyEqual(testCase, classNames(out), {'epochfiles_ingested'}, ...
    ['the class must NOT be ingestion_manifest yet -- the migrator header ' ...
     'records both blockers: the epoch mint, and epochprobemap having no home']);
end
...
% `verifySubstring` is the built-in function-based-test qualification
% (matlab.unittest), used above with its optional 4th diagnostic argument. It is
% deliberately NOT redefined here: a local function of that name would shadow the
% documented qualification for the whole file.
