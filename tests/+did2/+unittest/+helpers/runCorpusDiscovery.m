...
if strcmp(options.TargetVersion, 'V_eta')
    ...
    % #66 INCREMENT 1: decompose each ingested epoch's serialized `epochprobemap`
    ...
    %
    % ORDER: AFTER epochMint (it anchors to the `epoch` DOCUMENTS epochMint
    % appends -- a REAL dependence, not conventional; run before them it would
    % refuse every row `refused_no_epoch`) and AFTER resolveSessionAnchors (the
    % #30 observations it retires have had their session anchors folded, and it
    % mints its own 'during' anchor as a final `relative_reference` directly). It
    % commutes with the passes below it: it reads only `epochfiles_ingested`,
    % `epoch` and `subject` documents and writes only observations + one anchor
    % per epoch; none of them reads or writes those.
    %
    % READ ITS ZEROS WITH `epochfiles_ingested_seen` BESIDE THEM. Only PRED, Dab
    % and Soph carry ingested epochs at all, so `epochfiles_ingested_seen 0` is a
    % statement about the SAMPLE, not the pass. `probe_rows_total` is the fan-out
    % denominator; `observations_emitted` the fan-out.
    %
    % GUARDED like its siblings: writeCorpusReport is below and an uncaught throw
    % here costs the run its whole census. A failure leaves every document in its
    % pre-fold form, which is the state the corpus is green in today.
    result = did2.unittest.helpers.runBatchPass(result, ...
        'did2.convert.resolveEpochProbemap', 'epoch_probemap_fold', ...
        @(r) did2.convert.resolveEpochProbemap(r, ...
            'Validate', true, 'TargetVersion', options.TargetVersion));
    ...
    %
    % IT DELETES THE UNREFERENCED SOURCES, PER DOCUMENT (team, 2026-08-21). The
    % verify-before-delete is armed conditionally: a
    % `stimulus_response_scalar_parameters_basic` document is deleted only where
    % THIS run's own edge-walk proves it unreferenced, and kept where anything
    % still points at it -- so `parameters_documents_deleted` is what actually
    % went and `parameters_documents_unreferenced_after` is what was eligible.
    %
...
for k = 1:size(expected, 1)
    ...
    switch field
        ...
        case 'epoch_probemap_fold'
            fprintf('  %-34s inspected %d doc(s), %d unreadable\n', name, ...
                numGet(rep, 'documents_inspected'), ...
                numGet(rep, 'documents_unreadable'));
            % DENOMINATORS FIRST. `epochfiles_ingested_seen` gates everything
            % below it: 0 seen is a corpus with no ingested epochs (a fact about
            % the SAMPLE), and `probe_rows_total` is the fan-out denominator.
            fprintf(['      %d epochfiles_ingested seen; %d refused (no epoch ' ...
                     'doc); %d probe row(s) total\n'], ...
                numGet(rep, 'epochfiles_ingested_seen'), ...
                numGet(rep, 'refused_no_epoch'), ...
                numGet(rep, 'probe_rows_total'));
            % THE ROW PARTITION, never summed into probe_rows_total silently:
            % probe_rows_total = (rows that emitted) + no-subjectstring +
            % no-subject + unresolved-modality + image-deferred + stimulator.
            fprintf(['      rows: %d no-subjectstring, %d unresolved-subject, ' ...
                     '%d unresolved-modality, %d image-deferred, %d stimulator\n'], ...
                numGet(rep, 'rows_no_subjectstring'), ...
                numGet(rep, 'refused_no_subject'), ...
                numGet(rep, 'rows_unresolved_modality'), ...
                numGet(rep, 'rows_image_deferred'), ...
                numGet(rep, 'rows_stimulator'));
            fprintf(['      emitted %d observation(s) + %d manipulation(s) ' ...
                     '(%d instrument-resolved, %d instrument-omitted); %d ' ...
                     'epoch anchor(s) minted; %d quarantined\n'], ...
                numGet(rep, 'observations_emitted'), ...
                numGet(rep, 'manipulations_emitted'), ...
                numGet(rep, 'instrument_resolved'), ...
                numGet(rep, 'instrument_omitted'), ...
                numGet(rep, 'epoch_anchors_minted'), ...
                numGet(rep, 'fold_quarantined'));
            % THE #30 REPLACEMENT, on its own line. A retirement happens only
            % where EXACTLY ONE #30 obs names the probe AND a replacement
            % survived validation; `retire_skipped_ambiguous` is the patch/sharp
            % two-observation case, kept rather than risked.
            fprintf(['      #30 replace: %d retired, %d anchor(s) removed, %d ' ...
                     'skipped-ambiguous, %d no-match\n'], ...
                numGet(rep, 'session30_observations_retired'), ...
                numGet(rep, 'session30_anchors_removed'), ...
                numGet(rep, 'retire_skipped_ambiguous'), ...
                numGet(rep, 'retire_no_match'));
            if numGet(rep, 'epochfiles_ingested_seen') == 0
                fprintf(['      (0 epochfiles_ingested -- the lines above are ' ...
                         'VACUOUS, not clean)\n']);
            end
        case 'response_parameters_fold'
            ...
            fprintf(['      parameters documents: %d seen, %d still ' ...
                     'referenced (KEPT), %d unreferenced (eligible); %d ' ...
                     'deleted (per-document, only what this run proved ' ...
                     'unreferenced)\n'], ...
