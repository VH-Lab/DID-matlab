...
%TEMPORAL_FREQUENCY_TUNING_CALC Brainstorm-J migrator: did_v1 temporal_frequency_tuning_calc -> the D-C analysis-tier
...
%   the calculator IS its result via superclass: a temporal_frequency_tuning_calc document carries both a
%   `temporal_frequency_tuning` result block (curve + interpretable scalars) and a `temporal_frequency_tuning_calc` calc
%   block (input_parameters + input dependency). This is the class the REAL corpus
%   stores (the bare `temporal_frequency_tuning` is rarely produced directly), so the decomposition
...
bodies = did2.convert.migrators_j.temporal_frequency_tuning(preBody);
bodies = jAttachCalcParams(bodies, preBody, 'temporal_frequency_tuning_calc');
end
