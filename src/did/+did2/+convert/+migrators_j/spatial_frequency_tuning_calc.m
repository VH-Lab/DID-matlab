...
%   ndi.calc.vis.spatialfrequency calculator OUTPUT document -> the
...
%   v1 `spatial_frequency_tuning` result block is RESHAPED into the one tuning_curve
...
%   read "the subject_calculation LEAF spatial_frequency_tuning_calculation" (and,
...
%
%       $ grep -n "jCalculation(preBody" spatial_frequency_tuning_calc.m
%         21:bodies = jCalculation(preBody, 'tuning_curve_calculation', 'tuning_curve', ...
%       $ find DID-schema/schemas/V_eta -name 'spatial_frequency_tuning_calculation.json'
%         (no match)
