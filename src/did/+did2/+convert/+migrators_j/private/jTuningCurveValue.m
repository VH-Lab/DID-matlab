...
axesList = {};
% `unit` is OPTIONAL on V_eta tuning_curve.value.independent_variables[]
% (mustBeNonEmpty: false per tuning_curve.json), so omit it here rather than
% minting an ontology_term whose node would be empty on every emission -- that
% would raise the #70 unminted-terms ratchet (did-schema/tools/
% check_empty_ontology_nodes.py) for no reader benefit. When the CURIEs for
% these five axes are minted, hand them back at this call site rather than
% widening the emitted body.
knownAxes = { ...
    'direction',            'direction'; ...
    'contrast',             'contrast'; ...
