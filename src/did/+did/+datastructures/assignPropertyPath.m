...
%
%   S = did.datastructures.assignPropertyPath(S, PROPERTYPATH, VALUE)
%
...
%
%   See also: subsasgn, isvarname

    parts = strsplit(char(propertyPath), '.');
    for p = 1:numel(parts)
        if ~isvarname(parts{p})
            error('did:datastructures:assignPropertyPath:invalidPropertyName', ...
